...
function Prepare({ client, wallets }) {
  ...
  async function request() {
    ...
    try {
      ...
      const receipt = await runContractTransaction({ invoke: () => client.request(wallet.address, record.credential_id, submittedRoot, record.schema_hash, Number(record.expires_ledger)), sign: (xdr) => wallets.sign(xdr, wallet.address), submit: client.submit, confirm: client.confirm, dispatch });
...
function Verify({ client }) {
  ...
  async function verify() {
    ...
    let record = null, status = null, authorized = null;
    if (client.configured) {
      try { [record, status] = await Promise.all([client.get(id), client.status(id)]); authorized = record?.issuer ? await client.is_authorized(record.issuer) : null; } catch { setMessage('RPC checks were unavailable. Local evidence is still shown separately.'); }
    }
